...
class BCVClient:
    ...
    def _parse_currency_value(self, val: str) -> Optional[float]:
        if not val:
            return None
    ...
    def _format_currency_ve(
        self, value: Optional[float], decimals: int = 4, thousands: bool = True
    ) -> Optional[str]:
        if value is None:
            return None
        fmt = f"{{:,.{decimals}f}}"
    ...
    def get_tasas(self, moneda: Optional[str] = None) -> Optional[Dict]:
        """
        ...
        try:
            ...
            if fecha_span and fecha_span.get("content"):
                ...
                if isinstance(content, str):
                    raw_iso = content.strip()
                elif content is None:
                    raw_iso = ""
                else:
                    # Si es una lista, tomar el primer elemento
                    raw_iso = content[0].strip() if content else ""
...
def get_tasas_bcv(
    moneda: Optional[str] = None, log_file: Optional[str] = None, log_level: int = logging.INFO
    ...
    result = client.get_tasas(moneda=moneda)
