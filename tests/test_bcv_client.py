...
def test_cli_with_python_m():
    ...
    result = subprocess.run(
        [sys.executable, "-m", "scraper_bcv", "--log-level", "INFO"], capture_output=True, text=True
    )
...
def test_cli_as_installed_command():
    ...
    # Intentar encontrar el comando scraper-bcv
    venv_bin = Path(sys.prefix) / "bin" / "scraper-bcv"

    if venv_bin.exists():
        # Usar el comando instalado en el venv
        cmd = [str(venv_bin), "--log-level", "INFO"]
    else:
        # Fallback: usar el módulo directamente
        cmd = [sys.executable, "-m", "scraper_bcv", "--log-level", "INFO"]

    result = subprocess.run(cmd, capture_output=True, text=True)

    # Verificar que no hubo error
    assert result.returncode == 0, f"Error ejecutando scraper-bcv: {result.stderr}"

    # Verificar que hay output informativo
    combined_output = result.stdout + result.stderr
    assert "INFO" in combined_output or "BCV" in combined_output
